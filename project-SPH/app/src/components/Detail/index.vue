...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      // 1、发请求，通知服务器用户想要的产品及数量
      //这里通过通过async，await获取向服务器提交数据究竟是成功了还是失败了的情况
      //根据成功与否决定是跳转路由还是告知用户加入购物车失败,即try，catch来操作成功/失败后的回调
      try {
        let result = await this.$store.dispatch('reqAddOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        this.$router.push({ name: 'addcartsuccess',query:{skuNum:this.skuNum} })
        //可以通过下面的这种方式传递参数，但是这会使得路由路径很丑，所以最好只传递一些简单参数
        //this.$router.push({name:'addcartsuccess'},query:{skuInfo:this.skuInfo})
        //所以为了兼顾内存和美观，这里选择会话存储（关闭网页后数据消失），本地存储只有5mb不考虑
  ...
  mounted() {
    ...
    //这里的skuid对应路由路径中接收的自定义id名，并且保存在params中
  },
};
</script>
