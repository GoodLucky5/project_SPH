...
export default {
  ...
  methods: {
    ...
    async open() {
      ...
      //需要知道支付成功还是失败，开启定时器不停的向后台确认
      if (!this.time) {
        this.timer = setInterval(() => {
          let result = this.$API.reqPayState(this.orderId);
          if (result.code == 200) {
            //付款成功
            clearInterval(this.timer);
            this.time = null;
            //保存支付成功返回的code值（用来判定用户点击已支付后是否跳转
